...
document.addEventListener('click', function(e) {
    if (e.target.tagName === 'IMG') {
       RenderedWebViewHeight.postMessage(document.getElementById('teXHTML').clientHeight);
        var imageUrl = e.target.src;
        var imageList = document.querySelectorAll('img'); // 获取所有图片
var index = Array.prototype.indexOf.call(imageList, e.target); // 获取当前点击图片的位置
var data = { 
  images: Array.prototype.map.call(imageList, function(img) {
    return img.src; // 构建图片集合
  }), 
  index: index, // 当前点击图片的位置
  url:imageUrl
};
      ImageClick.postMessage(JSON.stringify(data));
    }
  });